...
function parse_string(str, filename)
   local parser = Parser.new(mem.from_string(str, filename))
...
function selftest()
   ...
   local function test_string(src, exp)
      local parser = Parser.new(mem.from_string(src))
      parser:skip_whitespace()
   ...
   local function test_module(src, exp)
      local result = strip_locs(parse(mem.from_string(src)))
   ...
   parse(mem.from_string(require('lib.yang.ietf_inet_types_yang')))
   parse(mem.from_string(require('lib.yang.ietf_yang_types_yang')))
   parse(mem.from_string(require('lib.yang.ietf_softwire_common_yang')))
   parse(mem.from_string(require('lib.yang.ietf_softwire_br_yang')))
end
